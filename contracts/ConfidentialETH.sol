 ...
 * @notice Wraps native ETH into encrypted cETH
 */
contract ConfidentialETH is Ownable2Step {
    ...
    /**
     * @notice Wrap ETH to cETH (encrypted)
     */
    function wrap() external payable {
        require(msg.value > 0, "Must send ETH");
        
        euint256 amount = e.asEuint256(msg.value);
    ...
    /**
     * @notice Unwrap cETH back to ETH (plaintext amount for simplicity)
     */
    function unwrap(uint256 amount) external {
        euint256 eAmount = e.asEuint256(amount);
        ebool hasBalance = e.ge(balances[msg.sender], eAmount);
        
        // For unwrap, we need to verify balance
        // In production, this would use a callback pattern
